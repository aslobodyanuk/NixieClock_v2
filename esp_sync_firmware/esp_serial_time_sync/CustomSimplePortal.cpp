...
<form action="/timezone" method="POST">
    <a href="https://github.com/nayarsystems/posix_tz_db/blob/master/zones.csv">List of Timezones</a>
...
PortalCfg portalCfg;
static TimezoneCallback _SP_timezone_received = nullptr;
...
void SP_handleTimezone() {
  strcpy(portalCfg.timezone, _SP_server.arg("timezone").c_str());

  if (_SP_timezone_received) {
    _SP_timezone_received(portalCfg.timezone);
  }
}
...
void setTimeZoneCallback(TimezoneCallback callback) {
  _SP_timezone_received = callback;
}
...
void portalStart() {
  ...
  _SP_server.on("/timezone", HTTP_POST, SP_handleTimezone);
