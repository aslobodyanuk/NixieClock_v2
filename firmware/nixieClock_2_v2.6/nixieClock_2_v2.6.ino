// This version is actually installed on a clock right now. (even with all the properties)
...
/*
  ...
  - Управление эффектами и настройками В РЕЖИМЕ ЧАСОВ:
    - Удержание правой кнопки включает и выключает "глюки"
    ...
    - Удержание центральной кнопки включает режим настройки параметра сдвига времени (TIME_SHIFT)
      - Левая кнопка - выбор, центральная - "меньше", правая - "больше"
      - Клик "больше" и "меньше" - изменение параметра сдвига времени
      - Удержать "выбор" - возврат к режиму часов
      - При значении 0 функция сдвига времени не будет активна
      - Сдвиг времени делает поправку времени в RTC каждый день в 3:30:30
      - Время будет уменьшено или увеличено на TIME_SHIFT (секунд) - максимальное значение 9999 секунд, миниимальное -9999
      - Если часы спешат - установите значение меньше 0 (листайте вниз центральной кнопкой)
      - Если часы запаздывают - установите значение больше 0 (листайте вверх правой кнопкой)
*/
/*
  ...
  Версия 2.6:
  - Добавлено проигрывание анимации глюков при изменении режима через кнопку
  - Добавлена более частая синхронизация времени с RTC для повышения точности
  - Добавлен сдвиг времени для тех, у кого часы спешат\отстают
    - Сдвиг времени делает поправку времени в RTC каждый день в 3:30:30
    - Время будет уменьшено или увеличено на TIME_SHIFT (секунд) - максимальное значение 9999 секунд
    - Настройка параметра TIME_SHIFT в отдельное меню, смотрите раздел "Управление" выше
*/
...
#define DOT_BRIGHT 15       // яркость точки дневная (1 - 255)
#define DOT_BRIGHT_N 10     // яркость точки ночная (1 - 255)
...
#define BACKL_MIN_BRIGHT 3  // мин. яркость подсветки ламп в режиме дыхание (0 - 255)
...
// ==================  АНТИОТРАВЛЕНИЕ ====================
#define BURN_TIME 40        // период обхода индикаторов в режиме очистки, мс
...
int16_t TIME_SHIFT = 0;

#define SYNC_RTC_EVERY_MINUTES 10             // Синхронизация с RTC для повышения точности времени
